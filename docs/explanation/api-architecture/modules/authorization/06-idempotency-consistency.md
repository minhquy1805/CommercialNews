...
- downstream reporting/materialization truth

Therefore:

- governance correctness must come from Authorization truth
- audit is downstream evidence, not part of governance success
- caches/materializations may accelerate evaluation, but must not become authority
...
- uniqueness of active assignments and grants according to model policy
...
- invalidate or supersede by governance events/revisions
- prefer primary truth reads for admin governance confirmation flows
...
- optional downstream governance notifications if introduced by policy
...
- assigning a role the user already has returns stable success/no-op semantics if that is the chosen V1 rule
- granting a permission a role already has returns stable success/no-op semantics if that is the chosen V1 rule
...
- repeated revoke returns a deterministic result
- implementation must not mix no-op success and conflict arbitrarily for the same semantic operation

### 2.3 Implementation posture

Recommended implementation posture:

- enforce invariants with DB constraints where practical
- use deterministic no-op or conflict semantics consistently
- do not emit duplicate harmful downstream side effects for logically unchanged governance state
...
- explicit revision/version checks where needed
...
the outcome must still converge to one correct downstream state and zero duplicate harmful effects.
...
- admin reads after write must reflect the new state
...
- optional downstream governance notifications
...
- a permission snapshot must not show a grant before Authorization truth committed it
...
- activating/deactivating roles or permissions
...
- `ActivateRole` / `DeactivateRole`: role truth change + any required marker + Outbox
- `ActivatePermission` / `DeactivatePermission`: permission truth change + any required marker + Outbox
...
This matters because governance writes affect security enforcement and should remain predictable under load.
...
- Content business writes

Authorization may write only:
...
- optional downstream notification side effects
...
- consumed out of ideal operational sequence
...
- per role or permission aggregate for lifecycle updates where relevant

If ordered versioning exists:

- include subject/aggregate-scoped version in events
...
- constraints enforce convergence for uniqueness rules
- APIs return deterministic outcomes such as:
  - `applied`
  - `no-op`
  - `conflict`
  - policy-defined equivalent
...
- it must not reintroduce a revoked role or permission in derived state
...
- `Idempotency-Key` is recommended on governance write endpoints
...
- dedupe by `MessageId`
- use unique governance/audit event identifiers where applicable
...
- it must be invalidated or superseded by governance events/revisions
- it must fail closed when stale or uncertain
- it must not grant access solely from stale cached grants
- it should use revision/etag semantics per subject if practical
...
- policy deny spikes on admin endpoints
- outbox backlog/age for governance events
- audit ingestion lag/backlog and dead-letter rate where applicable
- duplicate-prevention indicators
  - dedupe hits
  - uniqueness conflicts
  - idempotency-key conflicts where supported
...
- `correlationId`
- `actorUserId`
- target identifiers such as `UserId`, `RoleId`, `PermissionId`
- stable outcome classification such as:
  - `applied`
  - `no-op`
  - `idempotent`
  - `conflict`
...
No unnecessary PII or secret material should be logged.

---

## 13) Summary

Authorization correctness in V1 rests on the following rules:
...
10. No global ordering or heterogeneous distributed transaction is assumed for Authorization workflows.  
