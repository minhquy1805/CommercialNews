...
public abstract class SeoReadingIntegrationEventHandler<TPayload>
    ...
    where TPayload : class
{
    ...
    protected SeoReadingIntegrationEventHandler(
    ...
    protected ISeoReadingEventIngestionService IngestionService { get; }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (JsonException exception)
        {
            _logger.LogWarning(
                exception,
                "Failed to deserialize SEO {EventDisplayName} reading payload. MessageId={MessageId}, EventType={EventType}",
                EventDisplayName,
                envelope.MessageId,
                envelope.EventType);
            ...
            return Result.Failure(
                Error.Validation(
                    ...
                    message: $"SEO {EventDisplayName} reading payload is invalid."));
        }
        ...
        if (payload is null)
        {
            return Result.Failure(
                Error.Validation(
                    ...
                    message: $"SEO {EventDisplayName} reading payload is required."));
        }

        SeoReadingEnvelopeContext context = SeoReadingEnvelopeContext.Create(
        ...
        if (result.IsFailure)
        {
            ...
            _logger.LogWarning(
                "Failed to ingest SEO {EventDisplayName} reading event. MessageId={MessageId}, EventType={EventType}, ErrorCode={ErrorCode}, ErrorMessage={ErrorMessage}, ErrorDetails={ErrorDetails}",
                EventDisplayName,
                envelope.MessageId,
                envelope.EventType,
                error.Code,
        ...
        _logger.LogInformation(
            "SEO {EventDisplayName} reading event ingested. MessageId={MessageId}, Applied={Applied}, Decision={Decision}, PreviousSourceVersion={PreviousSourceVersion}, IncomingSourceVersion={IncomingSourceVersion}",
            EventDisplayName,
            envelope.MessageId,
            applyResult.Applied,
    ...
    private string BuildPayloadErrorCode(string suffix)
    {
        ...
        if (eventKey.StartsWith("seo.", StringComparison.OrdinalIgnoreCase))
        {
            eventKey = eventKey.Substring("seo.".Length);
        }
        ...
        return $"READING.SEO.{eventKey}_{suffix}";
    }
}
