...
public abstract class InteractionReadingIntegrationEventHandler<TPayload>
    ...
    where TPayload : class
{
    ...
    protected InteractionReadingIntegrationEventHandler(
    ...
    protected IInteractionReadingEventIngestionService IngestionService { get; }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (JsonException exception)
        {
            _logger.LogWarning(
                exception,
                "Failed to deserialize interaction {EventDisplayName} reading payload. MessageId={MessageId}, EventType={EventType}",
                EventDisplayName,
                envelope.MessageId,
                envelope.EventType);
            ...
            return Result.Failure(
                Error.Validation(
                    ...
                    message: $"Interaction {EventDisplayName} reading payload is invalid."));
        }
        ...
        if (payload is null)
        {
            return Result.Failure(
                Error.Validation(
                    ...
                    message: $"Interaction {EventDisplayName} reading payload is required."));
        }

        InteractionReadingEnvelopeContext context =
            InteractionReadingEnvelopeContext.Create(
        ...
        if (result.IsFailure)
        {
            ...
            _logger.LogWarning(
                "Failed to ingest interaction {EventDisplayName} reading event. MessageId={MessageId}, EventType={EventType}, ErrorCode={ErrorCode}, ErrorMessage={ErrorMessage}, ErrorDetails={ErrorDetails}",
                EventDisplayName,
                envelope.MessageId,
                envelope.EventType,
                error.Code,
        ...
        _logger.LogInformation(
            "Interaction {EventDisplayName} reading event ingested. MessageId={MessageId}, Applied={Applied}, Decision={Decision}, PreviousInteractionStatsVersion={PreviousInteractionStatsVersion}, IncomingInteractionStatsVersion={IncomingInteractionStatsVersion}",
            EventDisplayName,
            envelope.MessageId,
            applyResult.Applied,
    ...
    private string BuildPayloadErrorCode(string suffix)
    {
        ...
        return $"READING.INTERACTION.{eventKey}_{suffix}";
    }
}
