...
public abstract class ContentReadingIntegrationEventHandler<TPayload>
    ...
    where TPayload : class
{
    ...
    protected ContentReadingIntegrationEventHandler(
    ...
    protected IContentReadingEventIngestionService IngestionService { get; }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (JsonException exception)
        {
            _logger.LogWarning(
                exception,
                "Failed to deserialize content {EventDisplayName} reading payload. MessageId={MessageId}, EventType={EventType}",
                EventDisplayName,
                envelope.MessageId,
                envelope.EventType);
            ...
            return Result.Failure(
                Error.Validation(
                    ...
                    message: $"Content {EventDisplayName} reading payload is invalid."));
        }
        ...
        if (payload is null)
        {
            return Result.Failure(
                Error.Validation(
                    ...
                    message: $"Content {EventDisplayName} reading payload is required."));
        }

        ContentReadingEnvelopeContext context = ContentReadingEnvelopeContext.Create(
        ...
        _logger.LogInformation(
            "Content {EventDisplayName} reading event ingested. MessageId={MessageId}, Applied={Applied}, Decision={Decision}, PreviousSourceVersion={PreviousSourceVersion}, IncomingSourceVersion={IncomingSourceVersion}",
            EventDisplayName,
            envelope.MessageId,
            applyResult.Applied,
    ...
    private string BuildPayloadErrorCode(string suffix)
    {
        ...
        return $"READING.CONTENT.{eventKey}_{suffix}";
    }
}
