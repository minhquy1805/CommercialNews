...
public abstract class InteractionAuditIntegrationEventHandler<TPayload>
    ...
    where TPayload : class
{
    ...
    protected InteractionAuditIntegrationEventHandler(
    ...
    protected IInteractionAuditEventIngestionService IngestionService { get; }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (JsonException exception)
        {
            _logger.LogWarning(
                exception,
                "Failed to deserialize interaction {EventDisplayName} audit payload. MessageId={MessageId}, EventType={EventType}",
                EventDisplayName,
                envelope.MessageId,
                envelope.EventType);
            ...
            return Result.Failure(
                Error.Validation(
                    ...
                    message: $"Interaction {EventDisplayName} audit payload is invalid."));
        }
        ...
        if (payload is null)
        {
            return Result.Failure(
                Error.Validation(
                    ...
                    message: $"Interaction {EventDisplayName} audit payload is required."));
        }

        var context = InteractionAuditEnvelopeContext.Create(
        ...
        if (result.IsFailure)
        {
            ...
            _logger.LogWarning(
                "Failed to ingest interaction {EventDisplayName} audit event. MessageId={MessageId}, EventType={EventType}, ErrorCode={ErrorCode}, ErrorMessage={ErrorMessage}",
                EventDisplayName,
                envelope.MessageId,
                envelope.EventType,
                error.Code,
        ...
        _logger.LogInformation(
            "Interaction {EventDisplayName} audit event ingested. MessageId={MessageId}, AuditId={AuditId}, WasInserted={WasInserted}, WasDeduped={WasDeduped}",
            EventDisplayName,
            envelope.MessageId,
    ...
    private string BuildPayloadErrorCode(string suffix)
    {
        ...
        return $"AUDIT.INTERACTION.{eventKey}_{suffix}";
    }
}