...
public sealed class GetAuditLogByMessageIdQueryHandler
    : IRequestHandler<GetAuditLogByMessageIdQuery, Result<AuditLogDetailResult>>
{
    private readonly IAuditLogRepository _auditLogRepository;
    private readonly IAuditResultMapper _auditResultMapper;

    public GetAuditLogByMessageIdQueryHandler(
    ...
    public async Task<Result<AuditLogDetailResult>> Handle(
        GetAuditLogByMessageIdQuery request,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(request);

        var auditLog = await _auditLogRepository.GetByMessageIdAsync(
            request.MessageId,
            cancellationToken);
