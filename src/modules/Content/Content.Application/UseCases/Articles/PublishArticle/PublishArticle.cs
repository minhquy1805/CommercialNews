...
namespace Content.Application.UseCases.Articles.PublishArticle
{
    public sealed class PublishArticleUseCase : IPublishArticleUseCase
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                return Result<PublishArticleResponseDto>.Failure(ContentErrors.Article.InvalidArticleId);
            }

            if (request.ExpectedVersion <= 0)
            {
                return Result<PublishArticleResponseDto>.Failure(ContentErrors.Article.InvalidVersion);
            }
            ...
            try
            {
                ...
                if (article is null)
                {
                    return Result<PublishArticleResponseDto>.Failure(ContentErrors.Article.NotFound);
                }
                ...
                try
                {
                    Article? updatedArticle = await _articleRepository.PublishAsync(
                    ...
                    if (updatedArticle is null)
                    {
                        ...
                        return Result<PublishArticleResponseDto>.Failure(ContentErrors.ConcurrencyConflict);
                    }
                    ...
                    return Result<PublishArticleResponseDto>.Success(new PublishArticleResponseDto
                    {
                        ArticleId = updatedArticle.ArticleId,
            ...
            {
                return Result<PublishArticleResponseDto>.Failure(MapPersistenceException(exception));
            }
            catch (ContentDomainException ex)
            {
                return Result<PublishArticleResponseDto>.Failure(MapDomainException(ex));
            }
        }
