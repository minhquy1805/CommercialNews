...
namespace Identity.Application.Validation.LoginHistory.GetMyLoginHistory;

public static class GetMyLoginHistoryValidator
{
    private const int MaxPageSize = 100;
    private const int DefaultPage = 1;
    private const int DefaultPageSize = 20;

    public static Error? Validate(GetMyLoginHistoryRequestDto? request)
    {
        if (request is null)
        {
